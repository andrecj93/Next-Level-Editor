...
test.describe("Checklist block", () => {
  ...
  test("Ctrl+Enter toggles the checklist item the caret sits in (keyboard path)", async ({
    page,
  }) => {
    const editor = page.locator(".editor-content").first();
    await editor.click();
    ...
    // The caret is already inside the item after typing; the mouse gutter is the
    // only other toggle path, so keyboard users depend on this.
