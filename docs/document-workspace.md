...
The complete feature build measured on 2026-09-23 with Node 24.14.1 on Windows has a 317.7 KiB gzip ES core and 42.7 KiB gzip CSS (360.4 KiB combined). The base writing branch measured 260.8 KiB combined. These totals include all document features, so they do not isolate the collaboration adapter's cost. The unsplit UMD bundle is 1,074.7 KiB gzip. Prefer the ES build for deferred heavy features.
