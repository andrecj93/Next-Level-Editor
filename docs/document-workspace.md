...
The complete feature build measured on 2026-09-23 with Node 24.14.1 on Windows has a 346.02 KiB gzip ES core and 44.26 KiB gzip CSS (390.27 KiB combined). The base writing branch reports 262.7 KiB combined. These totals include all document features, so they do not isolate the collaboration adapter's cost. The unsplit UMD bundle is 1,109.77 KiB gzip. Prefer the ES build for deferred heavy features.
...
Run `npx playwright test e2e/document-collaboration.spec.ts e2e/collaboration-rich-content.spec.ts --workers=1 --retries=0` for independent browser peers using the authenticated reference WebSocket server and durable file storage. Keyboard scenarios cover insert/delete, formatted-boundary paragraph splits, nested-list Enter and last-cell Tab during a partition, then reconnect and reload. Chromium and mobile WebKit pass these six cases. These are operation samples, not load, latency or exhaustive rich-node qualification. Comment move/delete/orphan behavior, rich paste and real IME concurrency still require further qualification. Browser offline emulation did not reliably suspend existing WebKit WebSockets; these experiments partition connections at the server instead.
