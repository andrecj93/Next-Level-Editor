...
The complete feature build measured on 2026-09-23 with Node 24.14.1 on Windows has a 312.6 KiB gzip ES core and 42.2 KiB gzip CSS (354.8 KiB combined). The base writing branch measured 249.3 KiB combined. These totals include all document features, so they do not isolate the collaboration adapter's cost. The unsplit UMD bundle is 1,071.3 KiB gzip. Prefer the ES build for deferred heavy features.
