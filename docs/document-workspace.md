...
**Known structural concurrency failure:** two additional deterministic cases currently fail in `collaborationRichContent.test.ts`. While disconnected, one author moves a commented paragraph or merges it into the preceding paragraph through the HTML operation bridge; the other extends the marked text. After reversed and duplicated delivery, both replicas agree but omit the extension. The tests require the complete passage, its anchor, undo/redo and reload, and remain failing release gates. Passing convergence checks alone does not establish preservation of editing intent. See [F07 / #139](https://github.com/andrecj93/Next-Level-Editor/issues/139). The collaboration implementation remains experimental pending this repair.

An isolated conversion probe of `loro-prosemirror@0.4.4` and `@automerge/prosemirror@0.2.0` also found misplaced edits on paragraph moves; neither is a verified replacement. The probe used distinct concurrent text in every paragraph to detect swaps that identical inserted text would conceal. Automerge preserved the sampled splits and joins, while Loro did not. These synthetic conversion results do not qualify either full editor, transport, history or production deployment.

Further qualification includes the host's custom schema/IME corpus, assistive technology, distributed storage ownership and load testing. Concurrent tracked-review decisions need their own operation model and remain disabled in shared sessions. No vendor account or paid infrastructure is required for the reproducible reference implementation.
