...
The complete feature build measured on 2026-09-23 with Node 24.14.1 on Windows has a 331.64 KiB gzip ES core and 42.75 KiB gzip CSS (374.39 KiB combined). The base writing branch reports 262.7 KiB combined. These totals include all document features, so they do not isolate the collaboration adapter's cost. The unsplit UMD bundle is 1,093.64 KiB gzip. Prefer the ES build for deferred heavy features.
