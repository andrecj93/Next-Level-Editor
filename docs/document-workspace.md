...
| F09 / #141 | Editable sources and notes, repeated-note backlinks, APA author-date or numbered citations, regenerated bibliography, host formatter | Explicit book/web-page data only; no automatic bibliographic lookup. Unsupported rich media in reference text is plain text. |
| F10 / #142 | A4/Letter, orientation, margins, header/footer/page numbers, PDF preview using the same bytes as download | PDF uses its own flow renderer. It does not promise pixel-identical Word/HTML pagination. |
...
The complete feature build measured on 2026-09-23 with Node 24.14.1 on Windows has a 317.8 KiB gzip ES core and 42.7 KiB gzip CSS (360.5 KiB combined). The base writing branch measured 260.8 KiB combined. These totals include all document features, so they do not isolate the collaboration adapter's cost. The unsplit UMD bundle is 1,074.8 KiB gzip. Prefer the ES build for deferred heavy features.
