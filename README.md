...
- **5,032 Unit Checks** - Across 394 files after the library build, with enforced coverage thresholds (70% lines/functions/statements, 65% branches). Bundle-dependent checks also run in the package build gate.
...
| --- | --- | --- | --- |
| **Core (ES)** | 961.4 KB | **233.7 KB** | On import |
...
| **UMD** | 1,702.0 KB | 508.1 KB | On import (no splitting) |

So a page that never opens a code block, never picks a colour and never
exports pays **275.6 KB gzipped** for the library's JS + CSS. Vue is an external
