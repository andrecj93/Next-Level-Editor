...
- **5,051 Unit Checks** - Across 395 files after the library build, with enforced coverage thresholds (70% lines/functions/statements, 65% branches). Bundle-dependent checks also run in the package build gate.
...
| --- | --- | --- | --- |
| **Core (ES)** | 962.8 KB | **234.3 KB** | On import |
...
| **UMD** | 1,703.0 KB | 508.6 KB | On import (no splitting) |

So a page that never opens a code block, never picks a colour and never
exports pays **276.2 KB gzipped** for the library's JS + CSS. Vue is an external
