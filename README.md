...
- **5,076 Unit Checks** - Across 397 files after the library build, with enforced coverage thresholds (70% lines/functions/statements, 65% branches). Bundle-dependent checks also run in the package build gate.
...
| --- | --- | --- | --- |
| **Core (ES)** | 962.7 KB | **234.1 KB** | On import |
...
| **UMD** | 1,704.0 KB | 509.0 KB | On import (no splitting) |

So a page that never opens a code block, never picks a colour and never
exports pays **276.1 KB gzipped** for the library's JS + CSS. Vue is an external
