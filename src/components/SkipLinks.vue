...
interface Props {
  ...
  /**
   * Whether the toolbar / footer landmarks actually exist. A skip link whose
   * target is absent (toolbar hidden via showToolbar/pill mode, footer hidden
   * at toolbarPosition="bottom") dangled — activating it announced "not found"
   * and moved no focus. Absent landmarks are dropped from the list entirely.
   */
  hasToolbar?: boolean;
  hasFooter?: boolean;
  /**
   * Accessible name for the skip-links landmark. Two editors on one page would
   * otherwise expose two navigation landmarks with the identical name "Skip
   * links"; the host passes a per-instance label to disambiguate them in an
   * AT landmark list.
   */
  label?: string;
}
...
const navLabel = computed(() => props.label);
