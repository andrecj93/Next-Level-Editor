      ...
      <div class="playhead-pill">
        ...
        >
          ...
          >
            <span
              class="playhead-dot"
              :class="{ 'is-saving': isSaving, 'is-error': saveStatus === 'error' }"
              aria-hidden="true"
            />
        ...
        >
          ...
          >
            ...
            <transition name="playhead-menu">
              <!-- A disclosure GROUP, not a menu: 20+ swatch buttons are not
                   menu items, and declaring role=menu here without menuitem
                   children (or the keyboard model) is the R23-24 defect.
                   Mirrors EditorToolbar's colors panel. #R24-10 -->
              <div
...
const emit = defineEmits<{
  ...
  /**
   * One of the pill's menus opened or closed. The host needs this because the
   * pill teleports to <body>: its menus are unreachable from a query scoped to
   * the editor root, and the chrome must not recede under an open menu.
   */
...
/**
 * Tell the host whether a menu is open. The pill teleports to <body>, so its
 * menus live outside the editor root and the host cannot find them by scoping a
 * query to its own element — and scoping the query to the DOCUMENT instead
 * would let one editor's open menu freeze every other editor's chrome. The
 * state is already here and already reactive, so the pill simply says so.
 */
watch(
  () => openMenu.value !== null,
  (open) => emit("menu-open-change", open)
);
...
const toggleMenu = (id: MenuId) => {
  if (openMenu.value === id) {
    // Closing via the trigger goes through the focus-aware path too.
    closeOpenMenu();
    return;
  }
  ...
  openMenu.value = id;
  // A mouse switch (mousedown is prevented, so focus never moves to the new
  // trigger on its own) with keyboard focus inside the OLD menu would strand
  // focus on <body> when that menu unmounts. #R25-17
  if (hadFocusInside) void nextTick(() => openMenuTriggerFor(id)?.focus());
};

// --- Menu keyboard model (#R24-8/#R24-9) -------------------------------------
// The popups declare role=menu / menuitem / aria-haspopup, which is a keyboard
// CONTRACT, not decoration — the exact defect class R23-23 named. This is
// ToolbarDropdown's batch-124 model, ported: ArrowDown/Up on the trigger
// enters at first/last, arrows wrap, Home/End jump, Tab leaves and closes,
// and any close that happens with focus INSIDE the menu hands focus back to
// the trigger instead of dropping it on <body>.

/** The open menu's popup (only one can be open at a time). */
const openMenuEl = (): HTMLElement | null =>
  rootEl.value?.querySelector(".dropdown-menu.playhead-menu") ?? null;

const openMenuItems = (): HTMLElement[] =>
  Array.from(
    openMenuEl()?.querySelectorAll<HTMLElement>(
...
/**
 * The trigger button owning a popup. Scoped to the ACTIVE layer (all three
 * layers pre-render for the crossfade), and looked up FRESH each time — see
 * closeOpenMenu for why a captured element cannot be trusted across a patch.
 */
const openMenuTriggerFor = (id: MenuId): HTMLElement | null =>
  rootEl.value?.querySelector<HTMLElement>(
    `.playhead-layer.is-active [data-menu-trigger="${id}"]`
  ) ?? null;

/**
 * Close the open popup; when focus is inside it, return focus to the trigger.
 * A mouse user never focuses into the popup (@mousedown.prevent), so their
 * editor selection is left alone — same contract as ToolbarDropdown.close().
 */
const closeOpenMenu = () => {
  const menu = openMenuEl();
  ...
  const focusWasInside =
    ...
    menu.contains(document.activeElement);
  openMenu.value = null;
  // Restore AFTER the v-if flush, and re-query the trigger THEN: the patch
  // that unmounts the popup can replace the trigger node itself, so an
  // element captured before the flush may already be disconnected — focus()
  // on it is a silent no-op and focus falls to <body>.
  if (focusWasInside && id)
    void nextTick(() => openMenuTriggerFor(id)?.focus());
};

// The pill hides by unmounting its inner root (targetPoint -> null) while the
// COMPONENT stays mounted: an open menu would survive invisibly — its
// click-outside dismissal dead (rootEl is null), the host's recede-block flag
// stuck true, and the menu resurrecting OPEN on scroll-return after arbitrary
// outside interaction. A hidden pill has no menus. #R25-5
watch(targetPoint, (point) => {
  if (!point && openMenu.value !== null) closeOpenMenu();
});

/**
 * Rove-away parity for the colors DISCLOSURE (#R25-14): a keyboard user
 * Tabbing past the last swatch must not leave the popup floating orphaned.
 * Null relatedTarget is not a dismissal (a press on something non-focusable,
 * or the window losing focus) — the click-outside closer owns real outside
 * presses. Pick-driven focus moves are safe here: pickTextColor closes the
 * popup BEFORE its emit, so openMenu is already null when that focusout lands.
 */
const onColorsFocusOut = (event: FocusEvent) => {
  if (openMenu.value !== "colors") return;
...
const onTriggerKeydown = (id: MenuId, event: KeyboardEvent) => {
  ...
  void nextTick(() => {
    const items = openMenuItems();
    focusMenuItemAt(edge === "first" ? 0 : items.length - 1);
  });
};
...
const onMenuKeydown = (event: KeyboardEvent) => {
  ...
  switch (event.key) {
    ...
    case "End":
      ...
      break;
    case "Tab":
      // Tab leaves the popup entirely; close so it cannot linger open
      // behind the newly focused control. No focus return — Tab moves on.
      openMenu.value = null;
      break;
  }
};
...
const handleItemClick = (item: PlayheadMenuItem) => {
  ...
  // Close FIRST (queuing the focus restore), run the action a tick later: an
  // action that opens a modal must find focus already back on the trigger —
  // running it before the close made the modal capture <body> (the focused
  // item was unmounting) and Escape from the modal stranded focus. #R25-15
  closeOpenMenu();
  void nextTick(() => item.onClick?.());
};
...
const pickTextColor = (color: string) => {
  // Close BEFORE the emit: the host reacts by focusing the editor, and
  // deciding the restore after that always saw focus already outside — the
  // documented keyboard hand-back never fired integrated. #R25-16 #R24-9
  closeOpenMenu();
...
const handleKeydown = (event: KeyboardEvent) => {
  ...
  event.preventDefault();
  event.stopPropagation();
  // Restores focus to the trigger when it was inside the popup. #R24-9
  closeOpenMenu();
};
...
onBeforeUnmount(() => {
  // The menu-open watch cannot fire on teardown; tell the host explicitly so
  // its recede-blocking flag never outlives the menu. (The host also resets
  // on its own isPillMode flip — this covers every OTHER unmount path.) #R24-6
  if (openMenu.value !== null) emit("menu-open-change", false);
...
/* A failed save turns the ambient dot to the danger colour. #r20-2 */
.playhead-dot.is-error {
