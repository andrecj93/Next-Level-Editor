...
describe('ImageUploadModal', () => {
  ...
  describe('file upload validation', () => {
    const uploadFile = async (file: File) => {
      const wrapper = mount(ImageUploadModal, { props: { isOpen: true } })
    ...
    it('rejects a non-image file with an error and no preview', async () => {
      const wrapper = await uploadFile(
        new File(['x'], 'doc.pdf', { type: 'application/pdf' })
      )
      expect(wrapper.find('.error-message').text()).toContain('image file')
      expect(wrapper.find('.preview-section').exists()).toBe(false)
    })
    ...
    it('rejects an image over the 10 MB cap', async () => {
      ...
      const wrapper = await uploadFile(big)
