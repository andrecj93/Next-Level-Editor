...
describe('EditorToolbar', () => {
  ...
  const defaultProps = {
    ...
    listActions: [
      { id: 'bullet', label: 'Bullet List', icon: '<svg></svg>', tooltip: 'Bullet List', onClick: vi.fn(), isActive: vi.fn(() => false) }
    ],
    insertDropdownItems: [
    ...
    toolActions: [
      { id: 'clear', label: 'Clear', icon: '<svg></svg>', tooltip: 'Clear', onClick: vi.fn(), isActive: vi.fn(() => false) }
    ],
  ...
  describe('Component Rendering', () => {
    ...
    it('should render history controls (Undo/Redo)', () => {
      ...
      const undoButton = historyButtons.find(btn => btn.attributes('aria-label') === 'Undo')
      const redoButton = historyButtons.find(btn => btn.attributes('aria-label') === 'Redo')
    ...
    it('should render theme toggle button', () => {
      ...
      expect(themeToggle.attributes('aria-label')).toBe('Toggle dark/light theme')
    })

    it('should not render format HTML button in editor view', () => {
      expect(wrapper.findAll('.toolbar-btn-modern').some(btn => 
        btn.attributes('aria-label') === 'Format HTML'
      )).toBe(false)
    })

    it('should render format HTML button in code view', async () => {
      await wrapper.setProps({ viewMode: 'code' })
      const formatButton = wrapper.findAll('.toolbar-btn-modern').find(btn => 
        btn.attributes('aria-label') === 'Format HTML'
      )
  ...
  describe('History Controls', () => {
    it('should disable undo button when historyIndex is 0', async () => {
      ...
      const undoButton = wrapper.findAll('.toolbar-btn-modern').find(btn => 
        btn.attributes('aria-label') === 'Undo'
      )!
    ...
    it('should disable redo button when at latest history', async () => {
      ...
      const redoButton = wrapper.findAll('.toolbar-btn-modern').find(btn => 
        btn.attributes('aria-label') === 'Redo'
      )!
  ...
  describe('View Mode Toggle', () => {
    it('should mark editor button as active when viewMode is editor', () => {
      const editorButton = wrapper.findAll('.view-mode-btn').find(btn => 
        btn.text().includes('Editor')
      )!
    ...
    it('should mark code button as active when viewMode is code', async () => {
      await wrapper.setProps({ viewMode: 'code' })
      const codeButton = wrapper.findAll('.view-mode-btn').find(btn => 
        btn.text().includes('Code')
      )!
    ...
    it('should mark split button as active when viewMode is split', async () => {
      await wrapper.setProps({ viewMode: 'split' })
      const splitButton = wrapper.findAll('.view-mode-btn').find(btn => 
        btn.text().includes('Split')
      )!
    ...
    it('should mark preview button as active when viewMode is preview', async () => {
      ...
      const previewButton = wrapper.findAll('.view-mode-btn').find(btn => 
  ...
  describe('Props Reactivity', () => {
    it('should update textColor prop', async () => {
      await wrapper.setProps({ textColor: '#ff0000', showColorsDropdown: true })
      const colorPickers = wrapper.findAllComponents({ name: 'ColorPicker' })
    ...
    it('should update backgroundColor prop', async () => {
      await wrapper.setProps({ backgroundColor: '#00ff00', showColorsDropdown: true })
      const colorPickers = wrapper.findAllComponents({ name: 'ColorPicker' })
  ...
  describe('ColorPicker Events', () => {
    ...
    it('should emit background-color-change when background ColorPicker updates', async () => {
      ...
      expect(wrapper.emitted('background-color-change')?.[0]).toEqual(['#abcdef'])
    })
  })
