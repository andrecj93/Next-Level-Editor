...
describe("EditorPanels", () => {
  ...
  describe("viewMode: editor", () => {
    ...
    it.each(["input", "blur", "focus", "mouseup", "contextmenu", "paste", "drop"])(
  ...
  describe("viewMode: split", () => {
    ...
    it("the split editor pane honours editable=false (readonly not bypassed) (#r14b-4)", () => {
      const w = mountPanels({
        viewMode: "split",
        splitRightMode: "editor",
        editable: false,
      });
      // The split pane used to hard-code contenteditable="true", letting a
      // readonly editor be typed into via the split view.
      expect(
        w.get(".split-editor-panel .editor-content").attributes("contenteditable")
      ).toBe("false");
      w.unmount();
    });
