...
vi.mock('../TableModal.vue', () => ({
  default: { name: 'TableModal', template: '<div class="mock-table-modal"></div>', props: ['show'], emits: ['close', 'insert'] }
}))

vi.mock('../FindReplaceModal.vue', () => ({
  default: { name: 'FindReplaceModal', template: '<div class="mock-find-replace-modal"></div>', props: ['show', 'content'], emits: ['close', 'find', 'replace'] }
}))

vi.mock('../CodeBlockModal.vue', () => ({
  default: { name: 'CodeBlockModal', template: '<div class="mock-code-block-modal"></div>', props: ['show'], emits: ['close', 'insert'] }
}))

vi.mock('../TableDesigner.vue', () => ({
  default: { name: 'TableDesigner', template: '<div class="mock-table-designer"></div>', props: ['show', 'x', 'y'], emits: ['add-row-above', 'add-row-below', 'add-column-left', 'add-column-right', 'remove-row', 'remove-column', 'cell-properties', 'table-properties', 'delete-table'] }
}))

vi.mock('../TablePropertiesModal.vue', () => ({
  default: { name: 'TablePropertiesModal', template: '<div class="mock-table-properties-modal"></div>', props: ['show', 'mode', 'initialCellProps', 'initialTableProps'], emits: ['close', 'apply'] }
}))

vi.mock('../EmojiPicker.vue', () => ({
  default: { name: 'EmojiPicker', template: '<div class="mock-emoji-picker"></div>', props: ['show'], emits: ['select', 'close'] }
}))

vi.mock('../ImageUploadModal.vue', () => ({
  default: { name: 'ImageUploadModal', template: '<div class="mock-image-upload-modal"></div>', props: ['isOpen'], emits: ['close', 'insert'] }
}))

vi.mock('../EmbedModal.vue', () => ({
  default: { name: 'EmbedModal', template: '<div class="mock-embed-modal"></div>', props: ['isOpen'], emits: ['close', 'insert'] }
}))

vi.mock('../FileManagerModal.vue', () => ({
  default: { name: 'FileManagerModal', template: '<div class="mock-file-manager-modal"></div>', props: ['isOpen'], emits: ['close', 'insert'] }
}))

vi.mock('../TemplateModal.vue', () => ({
  default: { name: 'TemplateModal', template: '<div class="mock-template-modal"></div>', props: ['show'], emits: ['close', 'select'] }
}))

vi.mock('../HtmlCodeModal.vue', () => ({
  default: { name: 'HtmlCodeModal', template: '<div class="mock-html-code-modal"></div>', props: ['show', 'htmlContent'], emits: ['close'] }
}))

vi.mock('../CommandPalette.vue', () => ({
  default: { name: 'CommandPalette', template: '<div class="mock-command-palette"></div>', props: ['show', 'commands'], emits: ['close', 'execute'] }
}))
...
describe('ModalsContainer', () => {
  ...
  describe('Auto-save Indicator', () => {
    ...
    it('should show saved indicator with timestamp when lastSaved is set', () => {
      ...
      expect(indicator.find('.saved').text()).toContain(lastSaved.toLocaleTimeString())
    })
  ...
  describe('Props Passing', () => {
    it('should pass editorContent to FindReplaceModal', () => {
      ...
      const findReplaceModal = wrapper.findComponent({ name: 'FindReplaceModal' })
  ...
  describe('Event Emissions', () => {
    ...
    it('should emit find and replace events', async () => {
      ...
      expect(wrapper.emitted('replace')?.[0]).toEqual([{ from: 'old', to: 'new' }])
    })
    ...
    it('should emit image upload events', async () => {
      ...
      await modal.vm.$emit('insert', 'https://example.com/image.jpg', 'Alt text')
      expect(wrapper.emitted('insert-image')?.[0]).toEqual(['https://example.com/image.jpg', 'Alt text'])
