...
describe('HistoryTimeline', () => {
  ...
  describe('Timeline Entries', () => {
    ...
    it('should emit goToEntry when an entry is activated by keyboard', async () => {
      const wrapper = mount(HistoryTimeline, { props: defaultProps })
      // Entries are non-native controls; Enter/Space must activate them.
      await wrapper.findAll('.timeline-entry')[1].trigger('keydown', { key: 'Enter' })
      await wrapper.findAll('.timeline-entry')[0].trigger('keydown', { key: ' ' })

      expect(wrapper.emitted('goToEntry')?.[0]).toEqual([1])
      expect(wrapper.emitted('goToEntry')?.[1]).toEqual([0])
    })

    it('should expose entries as focusable buttons with current state', () => {
      const wrapper = mount(HistoryTimeline, { props: defaultProps })
      const entries = wrapper.findAll('.timeline-entry')

      entries.forEach((e) => {
        expect(e.attributes('role')).toBe('button')
        expect(e.attributes('tabindex')).toBe('0')
        expect(e.attributes('aria-label')).toBeTruthy()
      })
      // currentIndex is 2 in defaultProps.
