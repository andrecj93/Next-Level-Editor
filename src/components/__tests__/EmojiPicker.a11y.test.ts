...
describe("EmojiPicker dialog behavior", () => {
  ...
  it("labels every emoji and category button for screen readers", async () => {
    const w = await openPicker();
    // Category buttons announce their name + pressed state (not just a glyph).
    const categories = w.findAll(".category-btn");
    expect(categories.length).toBeGreaterThan(0);
    categories.forEach((c) => {
      expect(c.attributes("aria-label")).toBeTruthy();
      expect(c.attributes("aria-pressed")).toBeDefined();
    });
    // Emoji buttons announce their name, not the raw glyph.
    w.findAll(".emoji-btn").forEach((b) => {
      expect(b.attributes("aria-label")).toBeTruthy();
    });
  });

  it("has no corrupted (replacement-character) emoji glyphs", async () => {
    const w = await openPicker();
    // Regression: the "sleeping face" entry once held two U+FFFD bytes.
    const glyphs = w.findAll(".emoji-btn").map((b) => b.text());
    expect(glyphs.length).toBeGreaterThan(0);
    expect(glyphs.some((g) => g.includes("�"))).toBe(false);
