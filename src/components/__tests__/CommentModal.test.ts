...
describe("CommentModal mentions", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it("shows suggestions from the injected mentionSearch provider after the debounce", async () => {
    ...
    const wrapper = mount(CommentModal, {
      props: { isOpen: true, mentionSearch },
    });

    await typeInTextarea(wrapper, "Hello @a");
  ...
  it("clears suggestions when the provider rejects", async () => {
    const mentionSearch = vi.fn().mockRejectedValue(new Error("boom"));
    const wrapper = mount(CommentModal, {
      props: { isOpen: true, mentionSearch },
    });
    ...
    expect(mentionSearch).toHaveBeenCalled();
    expect(wrapper.find(".comment-mention-dropdown").exists()).toBe(false);
  });
  ...
  it("inserts the mention into the content when a suggestion is clicked", async () => {
    ...
    await typeInTextarea(wrapper, "Hello @al");
    await vi.advanceTimersByTimeAsync(150);
    await nextTick();
    ...
    expect(textarea.value).toBe("Hello @Alice Wonder ");
    expect(wrapper.find(".comment-mention-dropdown").exists()).toBe(false);
  });

  it("inserts the highlighted mention on Enter", async () => {
    const mentionSearch = vi.fn().mockResolvedValue(users);
    const wrapper = mount(CommentModal, {
      props: { isOpen: true, mentionSearch },
    });
    ...
    const textarea = wrapper.find("textarea")
      .element as HTMLTextAreaElement;
    expect(textarea.value).toBe("@Alice Wonder ");
  });
  ...
  it("submits the content including the selected mention", async () => {
    const mentionSearch = vi.fn().mockResolvedValue([users[0]]);
    const wrapper = mount(CommentModal, {
      props: { isOpen: true, mentionSearch },
    });
    ...
    await wrapper.find(".comment-modal-submit").trigger("click");
