...
describe('FileManagerModal', () => {
  ...
  describe('File Loading', () => {
    ...
    it('should display empty state when no files', () => {
      ...
      expect(wrapper.find('.drop-text').text()).toBe('Drag and drop files here')
    })
  ...
  describe('File Upload', () => {
    ...
    it('should show error count on upload failure', async () => {
      vi.mocked(fileManager.uploadFile).mockRejectedValueOnce(new Error('Upload failed'))
      wrapper = createWrapper({ isOpen: true })
      ...
      expect(wrapper.find('.error-message').text()).toContain('0 file(s) uploaded, 1 failed')
    })
    ...
    it('should show partial success message when some uploads fail', async () => {
      ...
      expect(wrapper.find('.error-message').text()).toContain('1 file(s) uploaded, 1 failed')
    })
  ...
  describe('File Deletion', () => {
    it('should delete selected files in batch', async () => {
      ...
      expect(fileManager.deleteFiles).toHaveBeenCalledWith(['file-1', 'file-2'])
    })
