...
const mountPill = (
  ...
  const { attach, attrs, ...propOverrides } = overrides;
  const inlineActions = propOverrides.inlineActions ?? makeInlineActions();
  const formatItems = propOverrides.formatItems ?? makeMenuItems("format");
  const insertItems = propOverrides.insertItems ?? makeMenuItems("insert");
  const overflowItems = propOverrides.overflowItems ?? makeMenuItems("overflow");
  const w = mount(PlayheadPill, {
    // Focus assertions need elements in the DOCUMENT: a detached tree never
...
describe("PlayheadPill", () => {
  ...
  describe("menus (Format / insert / overflow)", () => {
    ...
    it("implements the menu-button keyboard model it advertises (#R24-8)", async () => {
      // role=menu/menuitem + aria-haspopup promise WAI-ARIA menu keyboard
      // behavior — the exact contract R23-23 was about. ToolbarDropdown got
      // the real model in batch 124; the pill only got the roles.
      const { w } = mountPill({ state: "home", attach: true });
      const trigger = w.get('[aria-label="Paragraph format"]');

      await trigger.trigger("keydown", { key: "ArrowDown" });
      await nextTick();

      const items = w.findAll(".playhead-menu .dropdown-item");
      expect(items.length).toBeGreaterThan(0);
      expect(
        document.activeElement,
        "ArrowDown must open the menu AND focus its first item"
      ).toBe(items[0].element);

      // Wrap upward from the first item to the last.
      await items[0].trigger("keydown", { key: "ArrowUp" });
      const enabled = w
        .findAll(".playhead-menu .dropdown-item")
        .filter((i) => !(i.element as HTMLButtonElement).disabled);
      expect(document.activeElement).toBe(
        enabled[enabled.length - 1].element
      );

      // End/Home jump to the edges.
      await enabled[enabled.length - 1].trigger("keydown", { key: "Home" });
      expect(document.activeElement).toBe(enabled[0].element);
    });

    it("menu items are not tab stops (the menu owns its navigation)", async () => {
      const { w } = mountPill({ state: "home", attach: true });
      await w.get('[aria-label="Insert"]').trigger("click");

      for (const item of w.findAll(".playhead-menu .dropdown-item")) {
        expect((item.element as HTMLButtonElement).tabIndex).toBe(-1);
      }
    });

    it("Tab from inside the menu closes it", async () => {
      const { w } = mountPill({ state: "home", attach: true });
      const trigger = w.get('[aria-label="Insert"]');
      await trigger.trigger("keydown", { key: "ArrowDown" });
      await nextTick();
      expect(w.find(".playhead-menu").exists()).toBe(true);

      await w
        .findAll(".playhead-menu .dropdown-item")[0]
        .trigger("keydown", { key: "Tab" });
      await nextTick();

      expect(
        w.find(".playhead-menu").exists(),
        "an open menu must not linger behind the newly focused control"
      ).toBe(false);
    });

    it("Escape with focus inside the menu returns focus to the trigger (#R24-9)", async () => {
      const { w } = mountPill({ state: "home", attach: true });
      const trigger = w.get('[aria-label="Paragraph format"]');
      await trigger.trigger("keydown", { key: "ArrowDown" });
      await nextTick();
      expect(document.activeElement?.classList.contains("dropdown-item")).toBe(
        true
      );
      ...
      // Two ticks: the close flushes the v-if, THEN the restore re-queries and
      // focuses. Compare against a FRESH query — the patch that unmounts the
      // popup can replace the trigger node, so a pre-close element reference
      // is disconnected by now (that same staleness was the bug in the fix's
      // first draft).
      await nextTick();
      await nextTick();
      ...
      ).toBe(w.get('[aria-label="Paragraph format"]').element);
    });

    it("activating an item with the keyboard returns focus to the trigger (#R24-9)", async () => {
      const { w } = mountPill({ state: "home", attach: true });
      const trigger = w.get('[aria-label="Insert"]');
      await trigger.trigger("keydown", { key: "ArrowDown" });
      await nextTick();

      const first = w.findAll(".playhead-menu .dropdown-item")[0];
      ...
      expect(w.find(".playhead-menu").exists()).toBe(false);
      // Fresh query — see the Escape test for why the old reference is stale.
      expect(document.activeElement).toBe(
        w.get('[aria-label="Insert"]').element
      );
    });

    it("the colors popup is a labelled group, not a menu of non-menuitems (#R24-10)", async () => {
      // 20+ swatches with text-input pickers are a disclosure panel, not menu
      // items — EditorToolbar's identical popup was re-typed role=group in
      // batch 124 for exactly this reason.
      const { w } = mountPill({
        state: "home",
        attach: true,
        textColorPresets: ["#111111", "#222222"],
        highlightColorPresets: ["#ffff00"],
      });
      const trigger = w.get('[aria-label="Colors"]');
      expect(trigger.attributes("aria-haspopup")).toBeUndefined();
      expect(trigger.attributes("aria-expanded")).toBe("false");

      await trigger.trigger("click");
      const popup = w.get(".playhead-colors-menu");
      expect(popup.attributes("role")).toBe("group");
      expect(popup.attributes("aria-label")).toBe("Colors");
      expect(popup.find('[role="menuitem"]').exists()).toBe(false);
    });

    it("the colors popup closes when focus leaves the pill (#R25-14)", async () => {
      // The toolbar's disclosure got rove-away parity in batch 131; the
      // pill's twin never did — a keyboard user Tabbing past the last swatch
      // left the popup floating orphaned with aria-expanded=true.
      const { w } = mountPill({
        state: "home",
        attach: true,
        textColorPresets: ["#111111"],
        highlightColorPresets: ["#ffff00"],
      });
      await w.get('[aria-label="Colors"]').trigger("click");
      const swatch = w.get(".playhead-colors-menu .colors-swatch");
    ...
    it("the colors popup survives a focusout with no relatedTarget (#R25-14)", async () => {
      // Null relatedTarget = a press on something non-focusable (or the
      // window losing focus) — not a dismissal signal; the click-outside
      // closer owns real outside presses.
      const { w } = mountPill({
        state: "home",
        attach: true,
        textColorPresets: ["#111111"],
      ...
      expect(w.find(".playhead-colors-menu").exists()).toBe(true);
    });

    it("a modal-opening item's action runs AFTER focus is back on the trigger (#R25-15)", async () => {
      // item.onClick used to run first: it set the modal flag, the menu
      // unmounted the focused item, and the modal's activate() captured
      // <body> as previouslyFocused — so closing the modal stranded focus.
      const activeAtCall: (Element | null)[] = [];
      const items = makeMenuItems("insert");
      (items[0] as { onClick: () => void }).onClick = () => {
        activeAtCall.push(document.activeElement);
      };
      const { w } = mountPill({
        state: "home",
        attach: true,
      ...
      expect(activeAtCall).toHaveLength(1);
      expect(
        activeAtCall[0],
        "the action must see focus already restored, so a modal it opens captures the trigger"
      ).toBe(w.get('[aria-label="Insert"]').element);
    });

    it("keyboard swatch pick restores focus to the trigger despite the host's focus steal (#R25-16)", async () => {
      // Integrated, the color emit makes the host focus the EDITOR before the
      // close ran — focusWasInside was already false and the documented
      // restore never happened. The close decision now precedes the emit.
      ...
      const { w } = mountPill({
        ...
        attach: true,
        textColorPresets: ["#111111"],
        attrs: { onTextColorChange: () => editorStandIn.focus() },
      });
      await w.get('[aria-label="Colors"]').trigger("click");
      const swatch = w.get(".playhead-colors-menu .colors-swatch");
      (swatch.element as HTMLElement).focus();
      ...
      expect(
        document.activeElement,
        "the R24-9 contract: keyboard close paths land on the trigger"
      ).toBe(w.get('[aria-label="Colors"]').element);
    });

    it("switching menus by mouse with keyboard focus inside restores focus (#R25-17)", async () => {
      const { w } = mountPill({ state: "home", attach: true });
      const format = w.get('[aria-label="Paragraph format"]');
      await format.trigger("keydown", { key: "ArrowDown" });
      await nextTick();
      expect(document.activeElement?.classList.contains("dropdown-item")).toBe(
        true
      );

      // Mouse-click the OTHER trigger (mousedown is prevented, so focus never
      // moves there on its own) — the focused item dies with menu A's unmount.
      await w.get('[aria-label="Insert"]').trigger("click");
      await nextTick();
      await nextTick();
      ...
      expect(
        document.activeElement,
        "focus must not fall to <body> across a menu switch"
      ).toBe(w.get('[aria-label="Insert"]').element);
    });

    it("hiding the pill (anchor scrolled away) closes an open menu (#R25-5)", async () => {
      // targetPoint -> null unmounts the pill's inner root while the COMPONENT
      // stays mounted: the open menu survived invisibly (click-outside was
      // dead — rootEl null), kept the host's recede-block flag true, and
      // resurrected OPEN on scroll-return after arbitrary outside clicks.
      const { w } = mountPill({ state: "home", attach: true });
      await w.get('[aria-label="Insert"]').trigger("click");
      expect(w.emitted("menu-open-change")).toEqual([[true]]);

      // Editor scrolled fully above the viewport, past the 24px exit grace.
      await w.setProps({
        anchorRect: { top: -600, left: 100, width: 600, bottom: -60 },
      });
      await nextTick();
      expect(w.find(".playhead").exists(), "pill hidden").toBe(false);

      expect(
        w.emitted("menu-open-change"),
        "the host's recede-block flag must be released"
      ).toEqual([[true], [false]]);

      // Scroll back: the pill returns WITHOUT a spontaneously open menu.
      await w.setProps({
        anchorRect: { top: 40, left: 100, width: 600, bottom: 800 },
      });
      await nextTick();
      expect(w.find(".playhead").exists()).toBe(true);
      expect(w.find(".playhead-menu").exists()).toBe(false);
    });

    it("reports its open state to the host (#R23-59)", async () => {
      // The host cannot find these menus: the pill teleports to <body>, so a
      // query scoped to the editor root misses them — and a document-wide one
      // would match OTHER editors' pills and freeze their chrome too. So the
      // pill reports the state it already owns.
      const { w } = mountPill({ state: "home" });
      ...
      await w.get('[aria-label="Insert"]').trigger("click");
      expect(w.emitted("menu-open-change")).toEqual([[true], [false]]);
    });

    it("does not re-report when one menu replaces another", async () => {
      // "A menu is open" is what the host cares about; which one is its own
      // business. Hence the watch observes the boolean, not the menu id.
      const { w } = mountPill({ state: "home" });
