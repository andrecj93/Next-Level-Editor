<template>
  ...
  >
    ...
    <!-- A real listbox: the host binds the combobox trio (haspopup/expanded/
         controls/activedescendant) on the editing surface via the exposed
         listboxId/activeOptionId, exactly like the slash menu (#R23-5). #R23-54 -->
    <div
      :id="listboxId"
...
// The combobox bindings the HOST puts on its editing surface. #R23-54
const listboxId = nextInstanceToken("nle-vac");
const optionId = (index: number): string => `${listboxId}-opt-${index}`;
const activeOptionId = computed(() =>
  props.isOpen && filteredVariables.value.length > 0
    ? optionId(selectedIndex.value)
    : undefined
);

// `optionCount` lets the host announce "N suggestions available" in the live
// region — the replacement for the invalid `aria-expanded` that used to be the
// only open-state signal on the editing surface. #R32-1
const optionCount = computed(() => filteredVariables.value.length);

defineExpose({ handleEditorKeydown, listboxId, activeOptionId, optionCount });
</script>
