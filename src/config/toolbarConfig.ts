/**
 * Toolbar configuration and structure
 * Centralizes all toolbar items, actions, and layouts
 */
...
export interface ToolbarConfig {
  formatDropdown: DropdownItem[]
  inlineFormat: ToolbarAction[]
  alignment: DropdownItem[]
  fontSize: DropdownItem[]
  lists: ToolbarAction[]
  insert: DropdownItem[]
  tools: ToolbarAction[]
  productivity: DropdownItem[]
}

/**
 * Create toolbar configuration with all actions and handlers
 */
export function createToolbarConfig(handlers: {
  handleBlockAction: (tag: string) => void
  handleInlineAction: (tag: string) => void
  handleListAction: (tag: 'ul' | 'ol') => void
  handleTextAlignment: (alignment: string) => void
  ...
  isListActive: (tag: 'ul' | 'ol') => boolean
  isFontSize: (size: string) => boolean
  ...
  return {
    ...
    fontSize: [
      {
        ...
        isActive: () => isFontSize('small'),
      },
      {
        id: 'size-normal',
        label: 'Normal',
        onClick: () => handleFontSize('normal'),
        isActive: () => isFontSize('normal'),
      },
      {
        id: 'size-large',
        label: 'Large',
        onClick: () => handleFontSize('large'),
        isActive: () => isFontSize('large'),
      },
      {
        id: 'size-huge',
        label: 'Huge',
        onClick: () => handleFontSize('huge'),
        isActive: () => isFontSize('huge'),
      },
    ],
    ...
    tools: [
      ...
      },
      {
        ...
        icon: '<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M3 3h10v1H3V3zm0 3h10v1H3V6zm0 3h10v1H3V9zm0 3h6v1H3v-1z"/></svg>',
    ...
    productivity: [
      ...
      { divider: true } as DropdownItem,
      {
        id: 'spell-check',
        label: 'Toggle Spell Check',
        icon: '<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M2 2h1v12H2V2zm11 0h1v12h-1V2zM5 5h6v1H5V5zm0 3h6v1H5V8zm0 3h6v1H5v-1z"/></svg>',
        onClick: handleToggleSpellCheck,
      },
      { divider: true } as DropdownItem,
      {
