...
interface UseFormattingHandlersParams {
  performWithSelection: (action: (root: HTMLElement) => void, afterAction?: () => void) => void
...
export function useFormattingHandlers({
  ...
  const handleInlineAction = (tag: string) => {
    performWithSelection((root) => applyInlineStyle(root, tag), captureSnapshot)
  }
  ...
  const handleBlockAction = (tag: string, fallback = 'p') => {
    performWithSelection((root) => toggleBlock(root, tag, fallback), captureSnapshot)
  }
