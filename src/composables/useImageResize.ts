...
export function useImageResize(
  ...
  const clearAllImageSelections = () => {
    const selectedWrappers = editorContent.value?.querySelectorAll('.editor-image-wrapper.selected')
    if (selectedWrappers) {
  ...
  const handleImageClick = (event: Event) => {
    ...
    const isImageElement = target.classList.contains('editor-image-resizable') || 
  ...
  const doImageResize = (event: MouseEvent) => {
    ...
    const { img, startX, startY, startWidth, startHeight, position } = resizeData.value
