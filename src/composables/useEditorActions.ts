...
export function useEditorActions(options: UseEditorActionsOptions) {
  const { editorContent, performWithSelection, captureSnapshot, emit } =
    options;
  ...
  const handleExportPdf = async () => {
    if (!editorContent.value) return;
    try {
      emit("show-toast", "Generating PDF document...", "info");
      ...
      emit(
        "show-toast",
        "Failed to export PDF. Please check browser console for details.",
        "error"
      );
    }
  };
  ...
  const handleExportWord = async () => {
    if (!editorContent.value) return;
    try {
      emit("show-toast", "Generating Word document...", "info");
      await exportAsWord(editorContent.value.innerHTML);
      emit(
        "show-toast",
        "Word document exported successfully! Check your downloads folder.",
        "success"
      );
    } catch (error) {
  ...
  const handleCopyFormat = () => {
    ...
    if (!selection || selection.rangeCount === 0) {
      emit("show-toast", "Please select text to copy its format", "error");
      return;
    }
    ...
    if (result) {
      ...
      emit(
        "show-toast",
        "Format copied! Select text and use Paste Format to apply",
        "success"
      );
      ...
      emit("show-toast", "Could not copy format from selection", "error");
    }
  };
  ...
  const handlePasteFormat = () => {
    ...
    const selection = window.getSelection();
    if (!selection || selection.rangeCount === 0) {
      emit("show-toast", "Please select text to apply the format", "error");
      return;
    }
    ...
    if (success) {
      ...
      captureSnapshot();
      emit("show-toast", "Format applied successfully", "success");
      // Reset cursor
      if (editorContent.value) {
        editorContent.value.style.cursor = "";
      }
    } else {
      emit("show-toast", "No format copied or selection is empty", "error");
    }
  };
