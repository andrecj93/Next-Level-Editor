...
const BLOCK_ELEMENT_TAGS = new Set(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'blockquote'])
...
export function useKeyboardShortcuts(options: KeyboardShortcutsOptions) {
   ...
   */
  const findCurrentBlock = (startNode: Node): { block: HTMLElement | null; tag: string } => {
  ...
  const handleEnterKey = (event: KeyboardEvent) => {
    ...
    const { block: currentBlock, tag: currentBlockTag } = findCurrentBlock(range.startContainer)
   ...
   */
  const handleListIndentation = (event: KeyboardEvent, isShiftKey: boolean): boolean => {
