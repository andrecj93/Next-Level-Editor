...
describe("useFindReplace", () => {
  ...
  describe("handleFind", () => {
    // Editor-scoped navigation behavior (options, wrap, current/total,
    // page-scoping) is covered in useFindReplace.find.test.ts. These retain the
    // guard/robustness cases for this suite.
  ...
  describe("handleReplace", () => {
    ...
    it("replaces the match the user navigated to, not the first", () => {
      editorElement.innerHTML = "<p>cat cat cat cat cat</p>";
      ...
      // Find Next three times → the 3rd occurrence is the active match.
      handleFind({ findText: "cat", direction: "next", options });
      handleFind({ findText: "cat", direction: "next", options });
      const pos = handleFind({ findText: "cat", direction: "next", options });
      expect(pos).toEqual({ current: 3, total: 5 });

      handleReplace({ findText: "cat", replaceText: "dog", options });

      // The THIRD "cat" changed — the old code always replaced the first.
      expect(editorElement.innerHTML).toBe("<p>cat cat dog cat cat</p>");

      // The modal's follow-up Find Next must land on the match that slid into
      // the replaced slot (the former #4), not skip it.
      const after = handleFind({ findText: "cat", direction: "next", options });
      expect(after).toEqual({ current: 3, total: 4 });
    });

    it("falls back to the first match when Replace is used without navigating", () => {
      ...
      expect(editorElement.innerHTML).toBe("<p>dog cat cat</p>");
    });
  });
