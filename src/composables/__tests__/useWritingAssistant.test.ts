...
describe("useWritingAssistant", () => {
  ...
  describe("splitIntoSentences", () => {
    ...
    it("normalises EVERY occurrence of common abbreviations (global)", () => {
      const { splitIntoSentences } = useWritingAssistant();
      // Both "Mr." are protected now, so no spurious mid-name split.
      expect(
    ...
    it("does not split on decimals", () => {
      const { splitIntoSentences } = useWritingAssistant();
      expect(splitIntoSentences("The value is 3.14 today.")).toEqual([
  ...
  describe("splitIntoWords", () => {
    ...
    it("keeps accented letters intact (café is one 4-letter word)", () => {
      const { splitIntoWords } = useWritingAssistant();
      // The old ASCII \w regex clipped this to ["caf"] / split "naïve".
      expect(splitIntoWords("café naïve")).toEqual(["café", "naïve"]);
    });

    it("segments CJK text into words instead of dropping it", () => {
      const { splitIntoWords } = useWritingAssistant();
      // The old ASCII \w regex counted CJK as zero words.
      expect(splitIntoWords("日本語 test").length).toBeGreaterThan(1);
    });
  });
  ...
  describe("calculateStats", () => {
    it("computes every field for a known sentence", () => {
      ...
      expect(s.speakingTime).toBe(1); // ceil(10/130)
    });
    ...
    it("counts an emoji as a single character (grapheme, not UTF-16 units)", () => {
      const { calculateStats } = useWritingAssistant();
      const s = calculateStats("👍");
      expect(s.characters).toBe(1); // "👍".length === 2
      expect(s.charactersNoSpaces).toBe(1);
    });
  });

  // ============================================================
  // extractPlainText — block-aware, so multi-block HTML counts right
  // ============================================================
  describe("extractPlainText", () => {
    it("separates adjacent block elements so words/paragraphs aren't fused", () => {
      const { extractPlainText, calculateStats } = useWritingAssistant();
      const text = extractPlainText("<p>hello</p><p>world</p>");
      // Old textContent gave "helloworld" -> 1 word, 1 paragraph.
      const s = calculateStats(text);
      expect(s.words).toBe(2);
      expect(s.paragraphs).toBe(2);
    });

    it("treats <br> as a line break, not a word joiner", () => {
      const { extractPlainText, splitIntoWords } = useWritingAssistant();
      const text = extractPlainText("one<br>two");
      expect(splitIntoWords(text)).toEqual(["one", "two"]);
    });
  });
  ...
  describe("scheduleContentUpdate (debounced, lightweight)", () => {
    beforeEach(() => vi.useFakeTimers());
    afterEach(() => vi.useRealTimers());

    it("updateContent sets block-aware textContent and drives the stats computed", () => {
      const wa = useWritingAssistant();
      wa.updateContent("<p>hello</p><p>world</p>");
      // Block-aware extraction keeps the two words distinct.
      expect(wa.stats.value?.words).toBe(2);
      expect(wa.textContent.value).toContain("hello");
      expect(wa.textContent.value).toContain("world");
    });

    it("debounces: only the last content lands, after the delay", () => {
      const wa = useWritingAssistant();
      wa.scheduleContentUpdate("<p>first</p>");
      wa.scheduleContentUpdate("<p>second</p>");
      // Nothing applied synchronously (heavy work no longer runs per keystroke).
      expect(wa.textContent.value).toBe("");

      vi.advanceTimersByTime(300);
      expect(wa.textContent.value).toBe("second");
    });
  });
});
