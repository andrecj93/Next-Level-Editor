...
describe("useEditorEvents", () => {
  ...
  beforeEach(() => {
    ...
    // Setup mock functions. Default sanitizer is a passthrough so existing
    // clean-HTML assertions are unaffected; the code-view security test below
    // supplies a stripping sanitizer to prove it's actually applied.
  ...
  describe("onCodeBlur", () => {
    ...
    it("SECURITY: sanitizes code-view HTML before it re-enters the DOM or v-model", () => {
      const raw = '<img src=x onerror="alert(1)">';
      codeContent.value = raw;
      // A stripping sanitizer stands in for the real one.
      const stripping = vi.fn(() => "<p>clean</p>");

      const { onCodeBlur } = useEditorEvents({
        editorContent,
        codeContent,
        htmlContent,
        sanitizeHtml: stripping,
        captureSnapshot,
        updateFloatingToolbar,
        updateToolbarContext,
        rememberSelection,
      ...
      // The raw HTML went through the sanitizer, and the SANITIZED output — not
      // the raw string — is what reaches the DOM and the host model.
      expect(stripping).toHaveBeenCalledWith(raw);
      expect(editorElement.innerHTML).toBe("<p>clean</p>");
      expect(htmlContent.value).toBe("<p>clean</p>");
      expect(emit).toHaveBeenCalledWith("update:modelValue", "<p>clean</p>");
      expect(emit).not.toHaveBeenCalledWith("update:modelValue", raw);
    });
