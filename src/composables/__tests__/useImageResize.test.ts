...
describe("useImageResize", () => {
  ...
  describe("handleImageClick", () => {
    ...
    it("should not add duplicate handles if image already selected", () => {
      mockImg.click();
      const handlesBefore = mockWrapper.querySelectorAll(
        ".image-resize-handle"
      ).length;

      mockImg.click();
      const handlesAfter = mockWrapper.querySelectorAll(
        ".image-resize-handle"
      ).length;
  ...
  describe("startImageResize", () => {
    ...
    beforeEach(() => {
      imageResize.setupImageResizing();
      mockImg.click();
      handle = mockWrapper.querySelector(
  ...
  describe("Integration Tests", () => {
    it("should complete full resize workflow", () => {
      ...
      // 2. Start resize
      const handle = mockWrapper.querySelector(
        ".image-resize-handle.bottom-right"
      ) as HTMLDivElement;
      const mousedown = new MouseEvent("mousedown", {
        clientX: 100,
    ...
    it("should handle multiple images independently", () => {
      ...
      Object.defineProperty(img2, "offsetWidth", {
        value: 300,
        configurable: true,
      });
      Object.defineProperty(img2, "offsetHeight", {
        value: 150,
        configurable: true,
      });
