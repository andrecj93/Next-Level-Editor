...
describe("useAutoSave", () => {
  ...
  describe("Version History", () => {
    ...
    it("should throw error for invalid version index", async () => {
      ...
      expect(() => autoSave.rollbackToVersion(-1)).toThrow(
        "Invalid version index"
      );
      expect(() => autoSave.rollbackToVersion(10)).toThrow(
        "Invalid version index"
      );
    });
  });
  ...
  describe("Conflict Resolution", () => {
    ...
    it("should resolve conflict by accepting server version", async () => {
      ...
      const resolved = autoSave.resolveConflict(false); // Accept server

      // Must RETURN the server content so the host can load it into the editor —
      // without this the DOM kept local content and the next save silently
      // overwrote the server copy (data loss, the opposite of the user's choice).
      expect(resolved).toBe("server content");
      expect(autoSave.hasConflict.value).toBe(false);
      expect(autoSave.conflictInfo.value).toBeNull();
      expect(
        autoSave.saveHistory.value[autoSave.saveHistory.value.length - 1]
          .content
      ).toBe("server content");
    });

    it("returns null when keeping local changes (nothing to load)", async () => {
      const mockCallback = vi
        .fn()
        .mockResolvedValueOnce({ success: true, serverVersion: 1 })
        .mockResolvedValueOnce({
      ...
      expect(autoSave.resolveConflict(true)).toBeNull();
    });
  });
  ...
  describe("Save Status", () => {
    ...
    it('should return "saving" while save in progress', async () => {
      ...
      const mockCallback = vi.fn(
        () =>
          new Promise<{ success: boolean; serverVersion?: number }>(
            (resolve) => {
  ...
  describe("Failure surfacing (truthful Saved signal)", () => {
    it("marks status 'error' when the handler resolves { success: false }", async () => {
      const mockCallback = vi.fn(async () => ({ success: false }));
      const autoSave = useAutoSave(mockCallback);

      await autoSave.forceSave("content");

      // The save did NOT persist — the signal must not read 'saved'.
      expect(autoSave.saveStatus.value).toBe("error");
      expect(autoSave.lastError.value).toBeInstanceOf(Error);
      expect(autoSave.getStatistics().failedSaves).toBe(1);
      expect(autoSave.lastSaved.value).toBeNull();
    });
    ...
    it("surfaces a thrown handler error", async () => {
      ...
      expect(autoSave.saveStatus.value).toBe("error");
      expect(autoSave.lastError.value?.message).toBe("network down");
    });

    it("resolveConflict(false) advances to the server version (no re-conflict)", async () => {
      const mockCallback = vi.fn(async () => ({
        success: false,
        serverContent: "<p>server</p>",
        serverVersion: 7,
      }));
      const autoSave = useAutoSave(mockCallback);

      await autoSave.forceSave("<p>local</p>");
      expect(autoSave.hasConflict.value).toBe(true);
      expect(autoSave.conflictInfo.value?.serverVersion).toBe(7);

      autoSave.resolveConflict(false);
      expect(autoSave.currentVersion.value).toBe(7);
      expect(autoSave.hasConflict.value).toBe(false);
    });
  });
});
