...
describe("useHtmlSanitizer - variable pills", () => {
  ...
  it("regenerates the pill text from the validated name, discarding smuggled markup", () => {
    ...
    const pill = out.querySelector<HTMLElement>("span.editor-variable");
    // Canonical content since #R24-1: the regenerated label lives inside a
    // .variable-token child (print CSS hides it). Same guarantee, new shape.
    expect(pill?.textContent).toBe("{{ a }}");
    expect(
      pill?.querySelector(".variable-token")?.textContent
    ).toBe("{{ a }}");
