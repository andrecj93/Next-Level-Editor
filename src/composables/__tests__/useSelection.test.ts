...
describe('useSelection', () => {
  ...
  describe('performWithSelection', () => {
    ...
    it('should restore saved selection if available and no active selection', () => {
      const { rememberSelection, performWithSelection } = useSelection(editorContent)
    ...
    it('should handle action errors gracefully', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Formatting action failed', expect.any(Error))
      
      consoleWarnSpy.mockRestore()
    })

    it('should save new selection after action', () => {
      const { performWithSelection, saveSelection } = useSelection(editorContent)
      const actionMock = vi.fn(() => {
