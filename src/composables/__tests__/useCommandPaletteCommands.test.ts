...
describe('useCommandPaletteCommands', () => {
  ...
  describe('commands computed', () => {
    it('should return all 28 commands', () => {
      ...
      expect(commands.value).toHaveLength(28)
    })
    ...
    it('should include formatting commands', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const formattingCommands = commands.value.filter((c) => c.category === 'Formatting')
    ...
    it('should include structure commands (headings)', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const structureCommands = commands.value.filter((c) => c.category === 'Structure')

      expect(structureCommands).toHaveLength(3)
      expect(structureCommands.map((c) => c.id)).toEqual(['heading-1', 'heading-2', 'heading-3'])
    })
    ...
    it('should include list commands', () => {
      ...
      expect(listCommands.map((c) => c.id)).toEqual(['list-bullet', 'list-numbered'])
    })

    it('should include insert commands', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const insertCommands = commands.value.filter((c) => c.category === 'Insert')
    ...
    it('should include view commands', () => {
      ...
      expect(viewCommands.map((c) => c.id)).toEqual(['toggle-theme', 'fullscreen'])
    })

    it('should include export commands', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const exportCommands = commands.value.filter((c) => c.category === 'Export')
    ...
    it('should include history commands', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const historyCommands = commands.value.filter((c) => c.category === 'History')
  ...
  describe('command actions - document.execCommand', () => {
    ...
    it('should execute italic command', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const italicCommand = commands.value.find((c) => c.id === 'format-italic')
    ...
    it('should execute underline command', () => {
      const { commands } = useCommandPaletteCommands(mockCallbacks)
      const underlineCommand = commands.value.find((c) => c.id === 'format-underline')
