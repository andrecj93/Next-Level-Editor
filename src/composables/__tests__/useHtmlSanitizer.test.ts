...
describe('useHtmlSanitizer', () => {
  ...
  describe('Basic Sanitization', () => {
    ...
    it('should remove disallowed tags', () => {
      const html = '<p>Safe</p><script>alert("XSS")</script><style>body{color:red}</style>'
      const result = sanitizer.sanitizeHtml(html)
  ...
  describe('Allowed Tags', () => {
    ...
    it('should allow list tags', () => {
      const html = '<ul><li>Item 1</li><li>Item 2</li></ul><ol><li>Item A</li></ol>'
      const result = sanitizer.sanitizeHtml(html)
  ...
  describe('Attribute Sanitization', () => {
    ...
    it('should preserve existing rel attributes and add security tokens', () => {
      const html = '<p><a href="https://example.com" rel="external">Link</a></p>'
      const result = sanitizer.sanitizeHtml(html)
      expect(result).toContain('noopener')
      expect(result).toContain('noreferrer')
    ...
    it('should allow safe src in img tags', () => {
      const html = '<p><img src="https://example.com/image.jpg" alt="Image"></p>'
      const result = sanitizer.sanitizeHtml(html)
    ...
    it('should allow img alt, width, height attributes', () => {
      const html = '<p><img src="https://example.com/img.jpg" alt="Test" width="100" height="50"></p>'
      const result = sanitizer.sanitizeHtml(html)
    ...
    it('should allow table attributes', () => {
      const html = '<table border="1" cellpadding="5" cellspacing="0"><tbody><tr><td colspan="2">Cell</td></tr></tbody></table>'
      const result = sanitizer.sanitizeHtml(html)
    ...
    it('should normalize target attribute to _self or _blank', () => {
      const html = '<p><a href="https://example.com" target="_parent">Link</a></p>'
      const result = sanitizer.sanitizeHtml(html)
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle multiple dangerous elements', () => {
      const html = '<script>alert(1)</script><iframe src="evil"></iframe><embed src="bad">'
      const result = sanitizer.sanitizeHtml(html)
      expect(result).not.toContain('script')
