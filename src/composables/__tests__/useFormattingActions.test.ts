...
describe("useFormattingActions", () => {
  ...
  describe("handleCopyFormat", () => {
    ...
    it("should copy format from selection", () => {
      ...
      const selection = globalThis.getSelection();
      // Root travels along so detection stops at the editor boundary. #r16-6
      expect(formatPainter.copyFormat).toHaveBeenCalledWith(
        selection,
        editorContent.value
      );
    });
  ...
  describe("handlePasteFormat", () => {
    ...
    it("should paste format to selection", () => {
      ...
      const selection = globalThis.getSelection();
      // The editor root rides along so the paint applies per-block. #r15-5/6/8
      expect(formatPainter.pasteFormat).toHaveBeenCalledWith(
        selection,
        editorContent.value
      );
      expect(captureSnapshot).toHaveBeenCalled();
