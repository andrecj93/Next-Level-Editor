...
describe("useSmartAutocomplete - applyAutocomplete / handleInput", () => {
  ...
  describe("code context suppression", () => {
    /**
     * Build the editor with the caret inside a code context. Typographic
     * substitutions must NOT fire there: rewriting `a->b` into `a→b`, `--`
     * into an em dash, or linkifying a URL inside <pre><code> silently
     * corrupts the very code the user is documenting.
     */
    const createCodeEditor = (text: string, inline = false) => {
      const div = document.createElement("div");
      div.setAttribute("contenteditable", "true");
      document.body.appendChild(div);
    ...
    it("does not convert '--' inside a <pre><code> block", () => {
      const { div, editorRef } = createCodeEditor("SELECT 1 --");
      const { handleInput } = useSmartAutocomplete(editorRef);

      handleInput();

      expect(div.textContent).toBe("SELECT 1 --");
      expect(div.textContent).not.toContain(EMDASH);
    });

    it("does not convert '--' inside inline <code>", () => {
      const { div, editorRef } = createCodeEditor("i--", true);
      const { handleInput } = useSmartAutocomplete(editorRef);

      handleInput();

      expect(div.textContent).toBe("i--");
    });

    it("does not curl straight quotes inside <code>", () => {
      const { div, editorRef } = createCodeEditor('echo "hi', true);
      const { handleInput } = useSmartAutocomplete(editorRef);

      handleInput();

      expect(div.textContent).toBe('echo "hi');
      expect(div.textContent).not.toContain(LDQUO);
      expect(div.textContent).not.toContain(RDQUO);
    });

    it("does not auto-link a URL inside a <pre><code> block", () => {
      const { div, editorRef } = createCodeEditor("see https://ex.com ");
      const { handleInput } = useSmartAutocomplete(editorRef);

      handleInput();

      expect(div.querySelector("a")).toBeNull();
      expect(div.textContent).toBe("see https://ex.com ");
    });
  });
  ...
  describe("url and email auto-linking", () => {
    ...
    it("never builds live markup from text captured by a markdown shortcut", () => {
      // The replacement is assigned via innerHTML into the LIVE editor, before
      // the sanitizer (which only cleans the EMITTED string). Unescaped, this
      // built a real <img> whose inline onerror fired (self-XSS).
      const { div, editorRef } = createEditor("`<img src=x onerror=alert(1)>`");
      const { handleInput } = useSmartAutocomplete(editorRef);
      ...
      // No element was parsed out of the user's text — it stays literal text.
      expect(div.querySelector("img")).toBeNull();
      expect(code!.textContent).toBe("<img src=x onerror=alert(1)>");
    });

    it("renders a literal tag typed in inline code as text, not as markup", () => {
      const { div, editorRef } = createEditor("`<b>hi</b>`");
      const { handleInput } = useSmartAutocomplete(editorRef);

      handleInput();

      expect(div.querySelector("code b")).toBeNull();
      expect(div.querySelector("code")!.textContent).toBe("<b>hi</b>");
    });

    it("keeps entity-like sequences in a URL verbatim (no ©-style mangling)", () => {
      // The replacement is set via innerHTML, which decodes entities — an
      // unescaped "&copy" would render as "©" in the visible link text.
      const { div, editorRef } = createEditor("see https://ex.com/?a=1&copy=2 ");
      const { handleInput } = useSmartAutocomplete(editorRef);
      ...
      expect(anchor!.textContent).toBe("https://ex.com/?a=1&copy=2");
      expect(anchor!.getAttribute("href")).toBe("https://ex.com/?a=1&copy=2");
      expect(div.textContent).not.toContain("©");
    });
  });
