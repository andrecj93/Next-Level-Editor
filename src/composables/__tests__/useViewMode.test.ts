...
describe("useViewMode", () => {
  ...
  describe("View Mode Switching", () => {
    ...
    it("should allow all valid view modes", () => {
      const modes: Array<"editor" | "code" | "split" | "preview"> = [
        "editor",
        "code",
        "split",
        "preview",
  ...
  // A view-mode switch unmounts one editable surface and mounts a fresh, empty
  // one, which restoreEditorContent re-populates via innerHTML. Those are new
  // element instances: without re-initializing, an embed rendered fine but was
  // dead — no selection, no resize handles, Delete/resize did nothing — until a
  // full reload.
  describe("restoreEditorContent re-initializes embeds", () => {
    it("binds interactivity to embeds restored into a fresh surface", () => {
      const fresh = document.createElement("div");
      fresh.setAttribute("contenteditable", "true");
      document.body.appendChild(fresh);
      editorContent.value = fresh;
      htmlContent.value =
        '<div class="embedded-resizable-container" data-type="image"' +
        ' data-src="photo.png" data-width="400" data-height="300"' +
        ' data-maintain-aspect="true" data-alignment="center"' +
        ' contenteditable="false" tabindex="0">' +
        '<img src="photo.png" alt="photo"></div>';

      viewMode.restoreEditorContent();

      const container = fresh.querySelector<HTMLElement>(
        ".embedded-resizable-container"
      )!;
      expect(container).not.toBeNull();
      // Clicking selects it and draws the resize handles — proof the listeners
      // were bound to this new instance.
      container.dispatchEvent(new MouseEvent("click", { bubbles: true }));
      expect(container.querySelectorAll(".embed-resize-handle")).toHaveLength(4);
