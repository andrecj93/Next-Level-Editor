...
describe("useAdvancedKeyboardShortcuts", () => {
  ...
  it("keeps browser-conflicting Ctrl+Shift+I/T disabled even when wired", () => {
    const { registry } = useAdvancedKeyboardShortcuts(editor, {
      insertImage: vi.fn(),
      insertTable: vi.fn(),
    });
    // Wired, but stay off so DevTools / reopen-closed-tab aren't swallowed.
    expect(registry.getShortcut("insertImage")?.enabled).toBe(false);
    expect(registry.getShortcut("insertTable")?.enabled).toBe(false);
  });
