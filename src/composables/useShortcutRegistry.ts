...
/** Physical-key names (event.code) → the base character bindings are written
 *  with. Used to undo Shift's character substitution (see below). */
...
function eventToKeyString(event: KeyboardEvent): string {
  ...
  // With Shift held the browser reports the SHIFTED character in event.key
  // (Ctrl+Shift+8 → "*"), which can never match a binding written as
  // "ctrl+shift+8" — five advertised shortcuts (lists, super/subscript) were
  // permanently dead. Recover the physical base key from event.code for
  // digits and punctuation; letters already normalize via toLowerCase().
  if (event.shiftKey && event.code) {
    const digit = /^Digit(\d)$/.exec(event.code);
    if (digit) {
      key = digit[1];
    } else if (CODE_TO_BASE_KEY[event.code]) {
      key = CODE_TO_BASE_KEY[event.code];
    }
  }
...
function getPlatform(): "win" | "mac" | "linux" {
  // `navigator` is not a global in Node before v21, and this is reached during
  // SETUP — on Node 20 LTS a bare reference throws ReferenceError and fails the
  // whole server render. `platform` is deprecated and absent in some runtimes,
  // so fall back to userAgent before defaulting.
...
export function useShortcutRegistry() {
   ...
   */
  // localStorage, or null when unavailable (server render, sandboxed iframe,
  // storage disabled). Guarding keeps this composable — called in setup, so it
  // runs during SSR — from logging on every server render.
  ...
  const saveCustomBindings = () => {
    ...
    try {
      ...
      console.warn("Failed to save custom bindings:", error);
    }
  };
  ...
  const loadCustomBindings = () => {
    ...
    try {
      ...
      if (json) {
        ...
        // Validate the SHAPE, not just that it's JSON. A drifted entry (keys
        // not a string[]) used to be stored as-is, then every keydown threw
        // `activeKeys.some is not a function`, bricking the keyboard. Skip any
        // malformed entry instead of poisoning the registry.
        if (Array.isArray(data)) {
          for (const entry of data) {
            const shortcutId = (entry as CustomBinding)?.shortcutId;
            const keys = (entry as CustomBinding)?.keys;
            if (
      ...
      console.warn("Failed to load custom bindings:", error);
    }
  };
