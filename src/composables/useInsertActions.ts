...
import { insertLink as insertLinkUtil, insertImage as insertImageUtil } from '../utils/formatting'
import { insertHorizontalRule, insertTable as insertTableUtil } from '../utils/commands'
...
interface InsertActionsOptions {
  ...
  performWithSelection: (callback: (root: HTMLElement) => void, afterCallback?: () => void) => void
...
export function useInsertActions(options: InsertActionsOptions) {
  ...
  const insertLink = () => {
    ...
    if (url) {
      performWithSelection((root) => insertLinkUtil(root, url), captureSnapshot)
    }
  }
  ...
  const handleInsertImage = (url: string, alt: string) => {
    performWithSelection((root) => insertImageUtil(root, url, alt), captureSnapshot)
    closeImageUploadModal()
  }
