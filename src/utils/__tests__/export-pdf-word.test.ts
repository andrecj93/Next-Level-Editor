...
vi.mock('html-docx-js-typescript', () => ({
  asBlob: vi.fn(() => Promise.resolve(new Blob(['test'], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' })))
}))
...
describe('PDF and Word Export Tests', () => {
  ...
  describe('exportAsPdf', () => {
    ...
    it('exports PDF with custom filename', async () => {
      ...
      await expect(exportAsPdf(mockElement, 'custom.pdf')).resolves.not.toThrow()
    })
  })
  ...
  describe('exportAsWord', () => {
    ...
    it('handles HTML with various elements', async () => {
      const html = '<h1>Title</h1><p>Paragraph with <strong>bold</strong> and <em>italic</em> text.</p><ul><li>Item 1</li><li>Item 2</li></ul>'
