...
describe('Formatting Tests', () => {
  ...
  describe('outdentListItem', () => {
    ...
    it('inserts before the next sibling when the nested item is not last', () => {
      ...
      expect(result).toBe(true)
      const topItems = root.querySelector('ul')!.children
      // b is promoted after its grandparent li; c remains nested.
      expect(topItems[0].textContent).toContain('a')
      expect(topItems[1].textContent).toBe('b')
      expect(topItems[0].querySelector('ul')!.querySelector('li')!.textContent).toBe('c')
    })
