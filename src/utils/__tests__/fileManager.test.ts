...
describe('FileManagerService', () => {
  ...
  describe('File Upload', () => {
    ...
    it('stores the file when the thumbnail cannot be decoded', async () => {
      // A browser that cannot decode the image (a .heic in Chrome, a corrupt
      // .png) makes createThumbnail reject — but the FILE is fine, so the upload
      // must succeed with the thumbnail simply skipped. It used to reject the
      // whole upload with a bare "Upload failed". #R23-16
      stubImage(() => new FailingImage())

      const imageFile = new File(['broken'], 'broken.png', { type: 'image/png' })
      const result = await fileManager.uploadFile(imageFile)

      expect(result.name).toBe('broken.png')
      expect(result.thumbnail).toBeUndefined()
    })

    it('persists a file whose thumbnail generation fails', async () => {
      stubImage(() => new FailingImage())
      const imageFile = new File(['broken'], 'broken.png', { type: 'image/png' })

      const result = await fileManager.uploadFile(imageFile)

      // The file IS stored — only the thumbnail was skipped. #R23-16
      expect(fileManager.getFiles()).toHaveLength(1)
      expect(fileManager.getFiles()[0].id).toBe(result.id)
    })
  })
  ...
  describe('LocalStorage load edge cases', () => {
    ...
    it('warns (does not throw or error) when JSON parsing fails', () => {
      localStorage.setItem('next-level-editor-files', '{ not valid')
      const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
      const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

      expect(() => new FileManagerService()).not.toThrow()
      // A failed load is non-fatal (empty file list), so it's a warning — and
      // must never be a console.error that pollutes an SSR/import log.
