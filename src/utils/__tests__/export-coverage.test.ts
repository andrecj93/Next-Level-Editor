...
describe("Export Coverage Tests", () => {
  ...
  describe("exportAsHtml", () => {
    ...
    it("embeds checklist styling so an exported checklist keeps its checkboxes", async () => {
      exportAsHtml(
      ...
      // The editor draws checkboxes with CSS (no live <input>); export must carry
      // the same rules or the checked/unchecked state is lost.
      expect(text).toContain("ul.checklist");
      expect(text).toContain('li[data-checked="true"]::before');
    });
  });
