...
describe('Formatting multi-block behaviour', () => {
  ...
  describe('selection crossing a table boundary', () => {
    it('bolding from a paragraph into a table cell never splits or wraps the table', () => {
      root.innerHTML =
        '<p>before</p><table><tbody><tr><td>cell</td></tr></tbody></table>'
      const p = root.querySelector('p')!
      const cell = root.querySelector('td')!

      // Select from inside the paragraph into the cell's text.
      const range = document.createRange()
      range.setStart(p.firstChild!, 0)
      range.setEnd(cell.firstChild!, 4)
      const selection = window.getSelection()!
      selection.removeAllRanges()
      selection.addRange(range)
      ...
      // Cells hold text directly and aren't block tags, so this used to look
      // single-block and extractContents() reached across the table boundary,
      // splitting it and nesting table structure inside <strong>.
      expect(root.querySelectorAll('table').length).toBe(1)
      expect(root.querySelector('strong table')).toBeNull()
      expect(root.querySelector('strong tr, strong td')).toBeNull()
      // The table still holds its cell, and both texts survive.
      expect(root.querySelector('table td')).not.toBeNull()
