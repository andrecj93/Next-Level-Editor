...
describe('Selection Management', () => {
  ...
  describe('applyBackgroundColor', () => {
    ...
    it('"None"/transparent removes the highlight instead of painting invisible text', () => {
      // Highlight first.
      root.innerHTML = '<p>Test Text</p>'
      const p = root.querySelector('p')!
      const selectTest = () => {
        ...
        range.setEnd(root.querySelector('p')!.firstChild!, 4)
      ...
      selectTest()
      applyBackgroundColor(root, '#ffff00')
      expect(root.querySelector('span')!.style.backgroundColor).toBeTruthy()

      // Now select the highlighted run and clear it.
      const span = root.querySelector('span')!
      const clearRange = document.createRange()
      clearRange.selectNodeContents(span)
      const sel = window.getSelection()!
      sel.removeAllRanges()
      ...
      // The highlight span is gone (unwrapped), text preserved, and no forced
      // white text was left behind.
      expect(root.querySelector('span')).toBeNull()
      expect(root.textContent).toBe('Test Text')
      expect(root.innerHTML).not.toContain('color: rgb(255, 255, 255)')
