...
describe('Additional Commands Coverage', () => {
  ...
  describe('insertChecklist', () => {
    ...
    it('drops the checklist AFTER a surrounding list, not nested as a sublist', () => {
      root.innerHTML = '<ul><li>item</li></ul>'
      const li = root.querySelector('li')!
      ...
      // The checklist must be a sibling block of the original list, never an
      // indented sublist inside the <li>.
      expect(root.querySelector('li ul.checklist')).toBeNull()
      const checklist = root.querySelector('ul.checklist')
      expect(checklist).not.toBeNull()
      expect(checklist!.parentElement).toBe(root)
    })
  })
