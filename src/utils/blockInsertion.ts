...
export const splitBlockAtCaret = (
  ...
  // The caret sat at the END of the block, so there is no remainder to carry.
  // Cloning the block anyway produced an empty DUPLICATE — a blank
  // heading-sized line stamped with the SAME id (cloneNode copies attributes),
  // which made the TOC's own link ambiguous. A plain paragraph is the right
  // landing spot to keep typing in. #R23-38
  const carriesContent = Boolean(
    (tailContents.textContent || "").trim() ||
      (tailContents as unknown as ParentNode).querySelector?.(
        "img,br,table,ul,ol,hr,video,iframe"
      )
  );
  const tail = (
    carriesContent
  ...
  // An id belongs to exactly one element; never duplicate it into the tail.
