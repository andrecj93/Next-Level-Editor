...
<strong>enableVariables</strong> — <code v-pre>{{ mustache }}</code> template tokens rendered as pills.
        ...
        <p>
          Supplying your own variables replaces the built-in demo set, so a real
          app never ships the sample data:
        </p>
        <CodeBlock :code="variablesDataSnippet" lang="ts" />
        <CodeBlock :code="variablesSnippet" lang="vue" />
      </section>
...
The same component adapts to different jobs — an editor, a headless
      ...
      <section id="plugins" class="doc-section">
        ...
        <p>Register them with the <code>plugins</code> prop: <strong>slashCommands</strong> join the <code>/</code> menu, <strong>toolbarButtons</strong> the Tools menu, and <strong>commands</strong> the command palette.</p>
        <CodeBlock :code="pluginSnippet" lang="ts" />
        <CodeBlock :code="pluginUsageSnippet" lang="vue" />
...
// House style: template-only `vue` snippets. The variable shape goes in its own
// `ts` block, which also avoids embedding a closing script tag in a literal.
const variablesDataSnippet = `import { ref } from 'vue'

// Replaces the built-in demo fixtures. Reactive: change a value and the pills
// and the {{ autocomplete update with it.
const variables = ref([
...
const variablesSnippet = `<NextLevelEditor
  v-model="content"
  :enable-variables="true"
  :variables="variables"
/>`;
...
const pluginSnippet = `import { createPlugin, type PluginContext } from 'next-level-editor'

let editor: PluginContext | null = null

export const highlightPlugin = createPlugin({
  name: 'highlight',
  version: '1.0.0',
  install: (ctx) => { editor = ctx },
  toolbarButtons: [{
    id: 'highlight',
    label: 'Highlight',
    title: 'Highlight the selection',
    onClick: () => editor?.execCommand('hiliteColor', '#fef08a'),
  }],
  slashCommands: [{
    id: 'divider',
    trigger: 'divider',
    ...
    execute: () => editor?.execCommand('insertHorizontalRule'),
  }],
})`;

const pluginUsageSnippet = `<NextLevelEditor v-model="content" :plugins="[highlightPlugin]" />`;
